...
HUD 会按月自动顺延当前账期，并在 Token 汇总中显示“当前账期”用量。底部状态行会以 `YYYY/MM/DD-YYYY/MM/DD` 格式显示当前账期的首日和最后一日；内部统计使用下个账期开始日作为不包含的结束边界。

在实时 HUD 中也可以按 `p`，输入 `YYYY-MM-DD` 后回车保存当前账期开始日；按 Esc 取消。
...
手工订阅账期配置保存在 `~/.codex/codex-hud-subscription.json`。OpenAI 当前本地
telemetry 不提供可靠的月度订阅账期，因此该字段以手工设置为准。
